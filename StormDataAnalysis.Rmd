...
analData<-rawData %>% 
        select(STATE,EVTYPE,BGN_DATE,FATALITIES,INJURIES,PROPDMG) %>% 
        group_by(EVTYPE) %>% 
        summarise(MinDate = min(BGN_DATE), MaxDate = max(BGN_DATE),NumberofEvents = length(BGN_DATE),SumFatality = sum(FATALITIES), SumInjury = sum(INJURIES), SumPropertyDamage = sum(PROPDMG)) %>% 
        filter(SumFatality > 0 & SumInjury >0 & SumPropertyDamage >0)
...
kable(resultsData,format="markdown")

```


**Figure 1.** Summary of event type effect on human health likelihood (dimensionless). Values calculated as weighted sum of fatalities + 0.25*injuries/number of events. (Top 50 event types shown out of 76)
...
**Figure 2.** Summary of event type effect on property damage likelihood (dimensionless). (Top 50 event types shown out of 76)
